...
| Sampling | `Three options, pick whichever fits your stack. (1) MarketLens at 0x704E7a0e7864250303B05b25EabC2417CE99ceb6, which composes the reserve leg and the pool leg and enforces capacity. quoteBuy, quoteSell, quoteBuyExactOut, quoteSellExactOut, plus route, maxMint, redeemableAssets and brandForRedeem. EVERY function is view, so the whole surface is STATICCALL-able: batch it, wrap it, or sample it from inside an unlock you already hold. It does not call V4Quoter; it replays Pool.swap from extsload state with v4-core's own SwapMath and TickMath and applies the hook skim itself. MarketLens is not upgradeable, so a revision is a new address rather than new code behind the same one; read it from core.marketLens in deployments/asset-markets-mainnet-v6.json instead of hardcoding it. (2) Stock unmodified Uniswap V4Quoter at 0x6492C2e9340A6Cc1b12963D4723D819Af5B3CC5F for the pool leg. Exact, including our hook fee, but nonpayable, so top-level eth_call only. (3) Pure off-chain math: mint is 1:1, redeem is in minus floor(in*feeBps/10000), the v4 leg is full-range constant product with the LP fee on the whole input, then the hook takes floor(out*feePips/1e6) off the produced output. All three agree to the base unit: our fork test quotes 24 buy and 24 sell sizes across all six live markets against the canonical V4Quoter at the same block and requires exact equality. A quote is either a size that settles or a revert. We never return a silent haircut.` |
